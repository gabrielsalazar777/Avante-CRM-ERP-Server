...
//GET PROJECT DATES
router.get('/dates', isAuthenticated, (req, res) => {
   Project.find({
      $and: [{ startDate: { $exists: true } }, { endDate: { $exists: true } }],
   })
      .select('name startDate endDate color')
      .then((projectsWithDates) => {
         res.json(projectsWithDates);
      })
      .catch((err) => {
         console.log(err);
      });
...
router.post('/dates', isAuthenticated, (req, res) => {
   Project.findByIdAndUpdate(req.body.id, { color: req.body.color }, { new: true })
      .then((updatedProject) => {
         res.json(updatedProject);
      })
      .catch((err) => {
